...
(function initPoker() {
  ...
  let handsPlayed = 0;
  let phase = "idle"; // idle | dealt | drawn | finished
  let busy = false;
  let bestRank = 0;
  let winHands = 0;
  ...
  function renderCards(faceDown = false) {
    ...
    hand.forEach((c, i) => {
      ...
      if (faceDown) el.classList.add("back");
      else if (cardRed(c.s)) el.classList.add("red");
      el.dataset.idx = String(i);
      el.setAttribute("aria-label", faceDown ? "Карта" : cardLabel(c));
      if (!faceDown) {
        const rank = document.createElement("span");
        rank.className = "poker-card-rank";
        rank.textContent = RANK_CHARS[c.r - 2];
        const suit = document.createElement("span");
        suit.className = "poker-card-suit";
  ...
  function updateHud() {
    ...
    if (msgEl && phase === "idle") msgEl.textContent = "Пара или лучше — очки ауры";
  }
  ...
  function finishSession() {
    ...
    if (reward.won) {
      ...
      if (typeof burstParticles === "function") burstParticles(20);
    } else {
  ...
  function afterEvaluate(result) {
    ...
    updateHud();
    setBtnLabel();
    if (handsPlayed >= POKER_MAX_HANDS) finishSession();
  }
